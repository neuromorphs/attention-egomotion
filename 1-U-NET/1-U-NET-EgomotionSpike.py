...
################################
numSteps = 10
neuralNetwork = ModelSpike(numSteps).to(device)
...
def trainingLoop():
    for epoch in range(10):
        ...
        for i, (source, target) in enumerate(datasetDL):
            ...
            spikePred, memPred = neuralNetwork(source)
            loss = torch.zeros((1), dtype=torch.float, device=device)
            for step in range(numSteps):
                loss += lossFunction(memPred[step], target)
        ...
        torch.save(neuralNetwork.state_dict(), '../model/egomotionSpike.pt')
...
def inferenceLoop(interval, live=True):
    ...
    for i, (source, target) in enumerate(datasetDL):
        ...
        source, target = source.to(device), target.to(device)
        _, targetPred = neuralNetwork(source)

        targetPred = targetPred[-1].squeeze(1)
    ...
    if live is False:
        torch.save(dataset, '../dataset/egomotion/sphereInferenceSpike.pt')
    else:
...
if __name__ == '__main__':
    ...
    # trainingLoop()
    parameters = torch.load('../model/egomotionSpike.pt', map_location=device)
    neuralNetwork.load_state_dict(parameters)

    inferenceLoop(10, live=True)
